...
def getPositions():
    threading.Timer(5.0, getPositions).start()
    ...
    doQuery(connection, "UPDATE ferries_hist SET geom = ST_SetSRID(ST_Point(longitude, latitude),4326);")
    doQuery(connection, "CREATE INDEX ferries_gix ON ferries_hist USING GIST (geom);")
